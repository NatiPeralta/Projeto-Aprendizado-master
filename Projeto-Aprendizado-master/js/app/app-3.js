...
function setAvailableQuestions(){
    const totalQuestion = jogoscien.length;
    for(let i=0; i<totalQuestion; i++){
        availableQuestions.push(jogoscien[i])
    }
}
...
function getNewQuestion(){
    ...
    questionNumber.innerHTML = "Pergunta " + (questionCounter + 1) + " de " + jogoscien.length;
...
function next(){
    if(questionCounter === jogoscien.length){
        jogosprtOver();
    } else {
        getNewQuestion();
    }
...
function quizResult(){
    resultBox.querySelector(".total-question").innerHTML = jogoscien.length;
    resultBox.querySelector(".total-attempt").innerHTML = attempt;
    resultBox.querySelector(".total-wrong").innerHTML = attempt - correctAnswers;
    const percentage = (correctAnswers/jogoscien.length)*100;
    resultBox.querySelector(".percentage").innerHTML = percentage.toFixed() + "%";
    resultBox.querySelector(".total-score").innerHTML = correctAnswers + " / " + jogoscien.length;
...
window.onload = function (){
    homeBox.querySelector(".total-question").innerHTML = jogoscien.length;
}