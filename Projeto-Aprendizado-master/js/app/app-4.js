...
function setAvailableQuestions(){
    const totalQuestion = jogoshis.length;
    for(let i=0; i<totalQuestion; i++){
        availableQuestions.push(jogoshis[i])
    }
}
...
function getNewQuestion(){
    ...
    questionNumber.innerHTML = "Pergunta " + (questionCounter + 1) + " de " + jogoshis.length;
...
function next(){
    if(questionCounter === jogoshis.length){
        jogosprtOver();
    } else {
        getNewQuestion();
    }
...
function quizResult(){
    resultBox.querySelector(".total-question").innerHTML = jogoshis.length;
    resultBox.querySelector(".total-attempt").innerHTML = attempt;
    resultBox.querySelector(".total-wrong").innerHTML = attempt - correctAnswers;
    const percentage = (correctAnswers/jogoshis.length)*100;
    resultBox.querySelector(".percentage").innerHTML = percentage.toFixed() + "%";
    resultBox.querySelector(".total-score").innerHTML = correctAnswers + " / " + jogoshis.length;
...
window.onload = function (){
    homeBox.querySelector(".total-question").innerHTML = jogoshis.length;
}