...
function setAvailableQuestions(){
    const totalQuestion = jogosmat.length;
    for(let i=0; i<totalQuestion; i++){
        availableQuestions.push(jogosmat[i])
    }
}
...
function getNewQuestion(){
    ...
    questionNumber.innerHTML = "Pergunta " + (questionCounter + 1) + " de " + jogosmat.length;
...
function next(){
    if(questionCounter === jogosmat.length){
        jogosprtOver();
    } else {
        getNewQuestion();
    }
...
function quizResult(){
    resultBox.querySelector(".total-question").innerHTML = jogosmat.length;
    resultBox.querySelector(".total-attempt").innerHTML = attempt;
    resultBox.querySelector(".total-wrong").innerHTML = attempt - correctAnswers;
    const percentage = (correctAnswers/jogosmat.length)*100;
    resultBox.querySelector(".percentage").innerHTML = percentage.toFixed() + "%";
    resultBox.querySelector(".total-score").innerHTML = correctAnswers + " / " + jogosmat.length;
...
window.onload = function (){
    homeBox.querySelector(".total-question").innerHTML = jogosmat.length;
}