...
function setAvailableQuestions(){
    const totalQuestion = jogosfilo.length;
    for(let i=0; i<totalQuestion; i++){
        availableQuestions.push(jogosfilo[i])
    }
}
...
function getNewQuestion(){
    ...
    questionNumber.innerHTML = "Pergunta " + (questionCounter + 1) + " de " + jogosfilo.length;
...
function next(){
    if(questionCounter === jogosfilo.length){
        jogosprtOver();
    } else {
        getNewQuestion();
    }
...
function quizResult(){
    resultBox.querySelector(".total-question").innerHTML = jogosfilo.length;
    resultBox.querySelector(".total-attempt").innerHTML = attempt;
    resultBox.querySelector(".total-wrong").innerHTML = attempt - correctAnswers;
    const percentage = (correctAnswers/jogosfilo.length)*100;
    resultBox.querySelector(".percentage").innerHTML = percentage.toFixed() + "%";
    resultBox.querySelector(".total-score").innerHTML = correctAnswers + " / " + jogosfilo.length;
...
window.onload = function (){
    homeBox.querySelector(".total-question").innerHTML = jogosfilo.length;
}